...
int main(void)
{
    ...
    my_gpio_exti_init();        /* GPIO中断模式的初始化 默认打开中断 */
    my_gpio_set_gic_callback(GPIO1_Combined_16_31_IRQn, gpio1_16_18_irqhandler);
    my_epit_init(66-1,5000, 0); /* EPIT1定时器的初始化，默认打开中断 */
    my_epit_set_gic_callback(EPIT1_IRQn, epit1_irqhander);
    while(1) 
    {
