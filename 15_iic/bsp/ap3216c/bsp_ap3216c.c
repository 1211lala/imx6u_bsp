...
void ap_write(uint8_t addr, uint8_t* Txbuf, uint8_t size);
void ap_read(uint8_t addr, uint8_t regaddr , uint8_t* Rxbuf, uint8_t size);
/*
...
unsigned char ap3216c_init(void)
{
	...
	my_delay_ms(50);													/* AP33216C复位至少10ms */
	...
	// buf[0] = AP3216C_SYSTEMCONG;
	// buf[1] = 0X04;

	// printf("%d\r\n", 1);
	// ap_write(AP3216C_ADDR, buf, 2);
	// my_delay_ms(50);
	...
	// ap_read(AP3216C_ADDR, AP3216C_SYSTEMCONG, &data, 1);
	// printf("%d\r\n", 4);
    printf("%d\r\n", data);
	if(data == 0X03)
		return 0;	/* AP3216C正常 	*/
...
void ap_write(uint8_t addr, uint8_t* Txbuf, uint8_t size)
{
    i2c_master_start(I2C1, addr, kI2C_Write);
    i2c_master_write(I2C1, Txbuf, size);
}
...
void ap_read(uint8_t addr, uint8_t regaddr , uint8_t* Rxbuf, uint8_t size)
{
    ...
    i2c_master_start(I2C1, addr, kI2C_Write);
	printf("%d\r\n", 6);
	i2c_master_write(I2C1, &regaddr, 1);
	printf("%d\r\n", 7);
	i2c_master_repeated_start(I2C1, addr, kI2C_Read);
	while(!(I2C1->I2SR & (1 << 1))){};/* 等待传输完成 */
	printf("%d\r\n", 8);
	i2c_master_read(I2C1, Rxbuf, size);
	printf("%d\r\n", 9);
}